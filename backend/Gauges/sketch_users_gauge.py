"""
Gauge for counting users with sketch uploads.
"""
import logging
...
class SketchUsersGauge:
    ...
    async def get_gauge_data(self, start_date: Optional[datetime] = None, end_date: Optional[datetime] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Found {user_count} users with sketch uploads{date_range}")
            ...
            logger.error(f"Error counting sketch users: {str(e)}")
            return GaugeResult(0, "Sketch Users", f"Error: {str(e)}").to_dict()